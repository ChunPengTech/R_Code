...
## 形成性结构，使用 weights = mode_B 进行估计，而反映性结构，使用 mode_A(composite() 函数中的默认设置)
simple_mm <- constructs(
  composite("X", multi_items("x", 1:5), weights = mode_B),
  composite("MA", multi_items("ma", 1:5), weights = mode_B),
  composite("MB", multi_items("mb", 1:6), weights = mode_B),
  composite("Y", multi_items("y", 1:6), weights = mode_B)
)

# --------- Create structural model ---------

simple_sm <- relationships(
  paths(from = "X", to = cc("MA, MB, Y")),
  paths(from = "MA", to = cc("MB, Y")),
  paths(from = "MB", to = "Y")
)
