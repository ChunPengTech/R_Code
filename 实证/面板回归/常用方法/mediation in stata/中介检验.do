...
local controls is CPI edu fdl con

****************** sobel *******************

// quietly 表示不显示逐步回归的结果
sgmediation lnc, mv(lnincome) iv(indifi) cv($controls iid1-iid31 iyear1-iyear10) quietly

// 等价于
sgmediation3 lnc, mv(lnincome) iv(indifi) cv($controls) quietly
...
program define compute_med, rclass
    ...
    reghdfe lnincome indifi $controls  ///
        , absorb(id year)
    local a_M = _b[indifi]

    // 2. x m - y
    reghdfe lnc indifi lnincome $controls ///
        , absorb(id year)
...
// 等价于
bootstrap indirect=r(ind_eff) direct=r(dir_eff), reps(100) seed(123): sgmediation3 lnc, mv(lnincome) iv(indifi) cv($controls)
estat bootstrap, all