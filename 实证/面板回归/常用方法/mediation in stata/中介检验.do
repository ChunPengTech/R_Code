...
sgmediation lnc, mv(lnincome) iv(indifi) cv(is CPI edu fdl con iid1-iid31 iyear1-iyear10) quietly

// 等价于
sgmediation3 lnc, mv(lnincome) iv(indifi) cv(is CPI edu fdl con) quietly
...
program define compute_med, rclass
    ...
    reghdfe lnincome indifi is CPI edu fdl con  ///
        , absorb(id year)
    local a_M = _b[indifi]

    // 2. x m - y
    reghdfe lnc2 indifi lnincome is CPI edu fdl con ///
        , absorb(id year)
...
// 等价于
bootstrap indirect=r(ind_eff) direct=r(dir_eff), reps(100) seed(123): sgmediation3 lnc, mv(lnincome) iv(indifi) cv(is CPI edu fdl con)
estat bootstrap, all